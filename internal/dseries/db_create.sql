...
CREATE VIEW IF NOT EXISTS bucket_time AS
    WITH last_bucket AS (
        ...
        LIMIT 1)
    SELECT *,
           cast(strftime('%Y', created_at, '+3 hours') AS INTEGER) AS created_at_year,
           cast(strftime('%m', created_at, '+3 hours') AS INTEGER) AS created_at_month,
           cast(strftime('%d', created_at, '+3 hours') AS INTEGER) AS created_at_day,
           cast(strftime('%H', created_at, '+3 hours') AS INTEGER) AS created_at_hour,
           cast(strftime('%M', created_at, '+3 hours') AS INTEGER) AS created_at_minute,
           cast(strftime('%Y', updated_at, '+3 hours') AS INTEGER) AS updated_at_year,
           cast(strftime('%m', updated_at, '+3 hours') AS INTEGER) AS updated_at_month,
           cast(strftime('%d', updated_at, '+3 hours') AS INTEGER) AS updated_at_day,
           cast(strftime('%H', updated_at, '+3 hours') AS INTEGER) AS updated_at_hour,
           cast(strftime('%M', updated_at, '+3 hours') AS INTEGER) AS updated_at_minute,
